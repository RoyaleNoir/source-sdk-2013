...
#include "tonemap_ps20.inc"
#include "tonemap_ps20b.inc"
#include "tonemap_ps30.inc"

BEGIN_SHADER( Tonemap, "Help for the Tonemap shader." )
	...
	SHADER_DRAW
	{
		...
		SHADOW_STATE
		{
			...
			{
				DECLARE_STATIC_PIXEL_SHADER(tonemap_ps30);
				SET_STATIC_PIXEL_SHADER(tonemap_ps30);
			}
			else if (g_pHardwareConfig->SupportsPixelShaders_2_b())
			{
				DECLARE_STATIC_PIXEL_SHADER(tonemap_ps20b);
				SET_STATIC_PIXEL_SHADER(tonemap_ps20b);
			}
			else
			{
				DECLARE_STATIC_PIXEL_SHADER(tonemap_ps20);
				SET_STATIC_PIXEL_SHADER(tonemap_ps20);
			}
		}
		...
		DYNAMIC_STATE
		{
			...
			if (g_pHardwareConfig->SupportsShaderModel_3_0())
			{
				DECLARE_DYNAMIC_PIXEL_SHADER(tonemap_ps30);
				SET_DYNAMIC_PIXEL_SHADER(tonemap_ps30);
			}
			else if (g_pHardwareConfig->SupportsPixelShaders_2_b())
			{
				DECLARE_DYNAMIC_PIXEL_SHADER(tonemap_ps20b);
				SET_DYNAMIC_PIXEL_SHADER(tonemap_ps20b);
			}
			else
			{
				DECLARE_DYNAMIC_PIXEL_SHADER(tonemap_ps20);
				SET_DYNAMIC_PIXEL_SHADER(tonemap_ps20);
			}
		}
