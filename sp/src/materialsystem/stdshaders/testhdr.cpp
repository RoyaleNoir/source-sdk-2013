...
#include "testhdr_ps20.inc"
#include "testhdr_ps20b.inc"

BEGIN_SHADER( TestHDR, "Help for the TestHDR shader." )
	...
	SHADER_DRAW
	{
		...
		SHADOW_STATE
		{
			...
			{
				DECLARE_STATIC_PIXEL_SHADER(testhdr_ps20b);
				SET_STATIC_PIXEL_SHADER(testhdr_ps20b);
			}
			else
			{
				DECLARE_STATIC_PIXEL_SHADER(testhdr_ps20);
				SET_STATIC_PIXEL_SHADER(testhdr_ps20);
			}
		}
		...
		DYNAMIC_STATE
		{
			...
			if (g_pHardwareConfig->SupportsPixelShaders_2_b())
			{
				DECLARE_DYNAMIC_PIXEL_SHADER(testhdr_ps20b);
				SET_DYNAMIC_PIXEL_SHADER(testhdr_ps20b);
			}
			else
			{
				DECLARE_DYNAMIC_PIXEL_SHADER(testhdr_ps20);
				SET_DYNAMIC_PIXEL_SHADER(testhdr_ps20);
			}
		}
